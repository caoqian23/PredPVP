...
'''
# RFECV
train_x=np.loadtxt("D:\\python\\pycharm\\PyCharm Community Edition 2020.3.5\\pythonProject\\model\\change5\\norm training_all feature.csv",delimiter=",")
train_y = [1 for i in range(int(train_x.shape[0]/2))]  # 250个1
train_y.extend([0 for i in range(int(train_x.shape[0]/2))])
train_y = np.array(train_y)  # 前250个是1，后250个是0

test_x=np.loadtxt("D:\\python\\pycharm\\PyCharm Community Edition 2020.3.5\\pythonProject\\model\\change5\\norm test_all feature.csv",delimiter=",")
...
#np.savetxt("D:\\python\\pycharm\\PyCharm Community Edition 2020.3.5\\pythonProject\\github\\RFECV_train.csv", transformed_train, delimiter=",")
#np.savetxt("D:\\python\\pycharm\\PyCharm Community Edition 2020.3.5\\pythonProject\\github\\RFECV_test.csv", transformed_test, delimiter=",")
...
print(transformed_train.shape)
np.savetxt("D:\\python\\pycharm\\PyCharm Community Edition 2020.3.5\\pythonProject\\github\\Var_train.csv", transformed_train, delimiter=",")
np.savetxt("D:\\python\\pycharm\\PyCharm Community Edition 2020.3.5\\pythonProject\\github\\Var_test.csv", transformed_test, delimiter=",")
...
# 这里的判别函数为F统计量
selector = SelectKBest(f_classif, k=500)    # k=500  我们想要选择的变量数
selector.fit(train_x, train_y) # 在训练集上训练
transformed_train = selector.transform(train_x) # 转换训练集
transformed_test = selector.transform(test_x) # 转换测试集
np.savetxt("D:\\python\\pycharm\\PyCharm Community Edition 2020.3.5\\pythonProject\\github\\F_score_train.csv", transformed_train, delimiter=",")
np.savetxt("D:\\python\\pycharm\\PyCharm Community Edition 2020.3.5\\pythonProject\\github\\F_score_test.csv", transformed_test, delimiter=",")
'''
